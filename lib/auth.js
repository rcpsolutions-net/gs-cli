        ...
        try {
           spinner.text = 'Step 1 of 1: Requesting API access token for clientId: ' + clientId;
            ...
            if (newToken) {
                ...
                console.log(`--- /workspaces Response.data contains ${workspaceResponse.data.length} workspaces.`);
                ...
                console.log(`Looking for workspace names matching: ${process.env.GREENSHADES_WORKSPACE_NAME}`);
                console.log('Available workspaces:');
                workspaceResponse.data.forEach(w => console.log(`- ${chalk.redBright(w.name)} (ID: ${chalk.whiteBright(w.workspaceId)})`));            
                ...
                if( fnd ) {
                    console.log('\n- Found workspace: ' + workspaceName + ' (ID: ' + workspaceId + ')\n');
